...
const Invoicing: React.FC<InvoicingProps> = ({ soldJobs, customers, onPrepareInvoice }) => {
    ...
    const card = "rounded-2xl border border-gray-200 bg-white shadow-sm p-4";
    ...
    const JobCard = ({ job }: { job: EstimateRecord }) => {
        ...
        return (
             <li className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50 p-4 rounded-lg shadow-sm border">
                <div className="mb-2 sm:mb-0">
                    <p className="font-semibold text-gray-800">{customer?.name || 'Unknown Customer'}</p>
    ...
    return (
        <>
            <div className="mx-auto max-w-4xl p-4 sm:p-6">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold">Customer Invoicing</h1>
                ...
                <div className="space-y-6">
                    ...
                    <div className={card}>
                        ...
                        {pending.length === 0 ? (
                            <p className="text-sm text-gray-500 mt-3 p-3">No jobs are currently waiting for an invoice.</p>
                        ) : (
                    ...
                    <div className={card}>
                        <h2 className={h2}>Completed Invoices ({invoiced.length})</h2>
                         {invoiced.length === 0 ? (
                            <p className="text-sm text-gray-500 mt-3 p-3">No invoices have been generated yet.</p>
                        ) : (
                            <ul className="mt-4 space-y-3">
                                {invoiced.map(job => (
                                    <li key={job.estimateNumber} className="flex justify-between items-center bg-green-50/50 p-4 rounded-lg border border-green-200 opacity-80">
                                        <div>
                                            <p className="font-semibold text-gray-800">{findCustomer(job.customerId)?.name || 'Unknown'}</p>
                                            <p className="text-sm text-gray-500">
